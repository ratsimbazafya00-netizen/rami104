...
const RamiTable = {
  ...
  renderHandAndZones() {
    ...
    // Nettoyage de sécurité (ghosts / placeholders orphelins d'un drag interrompu)
  ...
  bindCardReorder(el, handEl) {
    ...
    const getOrder = () =>
      [...handEl.querySelectorAll(":scope > .card.draggable, :scope > .card-placeholder")]
        .map(x => x.dataset.id)
        .filter(Boolean);

    const forceCleanHandClasses = () => {
      // Filet de sécurité : aucune carte ne doit rester grisée
      handEl.querySelectorAll(".card.dragging").forEach(c => c.classList.remove("dragging"));
      handEl.querySelectorAll(".card-placeholder").forEach(p => p.remove());
      document.querySelectorAll(".card-drag-ghost").forEach(g => g.remove());
      handEl.classList.remove("drop-target");
    };
    ...
    const cleanup = (commit = false) => {
      ...
      if (placeholder && placeholder.parentNode) {
        // Remettre la vraie carte à la place du placeholder
      ...
      el.style.display = "";
      forceCleanHandClasses();

      if (commit) {
        this.handOrder = getOrder();
      }
      ...
      // Re-rendu léger pour garantir un état propre (classes, pas de fantômes)
      setTimeout(() => {
        this.suppressNextCardClick = false;
        if (!this.handDrag && this.lastState) {
          this.renderHandAndZones();
        }
      }, 30);
    };
    ...
    const begin = (ev) => {
      ...
      // Placeholder de même taille → la carte d'origine disparaît du flux
      placeholder = document.createElement("div");
      placeholder.className = "card-placeholder";
      placeholder.dataset.id = cardId;
    ...
    el.addEventListener("pointerdown", (ev) => {
      ...
      // Tactile : appui long (scroll prioritaire)
      timer = setTimeout(() => {
        timer = null;
    ...
    el.addEventListener("pointermove", (ev) => {
      ...
      // Déplacer le placeholder (pas la carte cachée)
      if (!placeholder) return;
      const siblings = [...handEl.querySelectorAll(":scope > .card.draggable, :scope > .card-placeholder")]
        .filter(n => n !== placeholder && n !== el);
      const target = siblings.find(other => {
        const r = other.getBoundingClientRect();
        return ev.clientX < r.left + r.width / 2;
      });
      if (target) handEl.insertBefore(placeholder, target);
      else handEl.appendChild(placeholder);
    }, { passive: false });
